...
for dir in "${dirs[@]}"; do
  ...
  docker build -t "app-$name" --build-arg SERVER_DIR="$name" --network=host . -f ./remote/Dockerfile
done
exit 1
fi

echo "Building image for app: $1"
docker build -t "app-$1" --build-arg SERVER_DIR="$1" --network=host . -f ./remote/Dockerfile
exit 1
